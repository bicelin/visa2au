...
# Per-page branded social cards (og:image). Keyed by the page's relative path,
# value = filename in app/imgs/. Generated by scripts/build_page_cards.py.
_CARD_MAP_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                              "scripts", "page_card_map.json")
_PAGE_CARDS = {}
if os.path.isfile(_CARD_MAP_PATH):
    try:
        _PAGE_CARDS = json.load(open(_CARD_MAP_PATH, encoding="utf-8"))
    except Exception:
        _PAGE_CARDS = {}
...
def main() -> None:
    ...
    for path in sorted(files):
        ...
        # Per-page branded social card: if this page has a custom og card, point
